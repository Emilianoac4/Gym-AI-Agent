...
# Verificar si servidor est?? activo
...
# ==================== FASE 1: LOGIN ====================
Write-Host "`n[3] Iniciando sesi??n..." -ForegroundColor Yellow
$loginBody = @{
  email = $EMAIL
  password = $PASSWORD
} | ConvertTo-Json
...
$profileBody = @{
  ...
  availability = "4 d??as por semana"
  dietPrefs = "Rico en prote??na"
} | ConvertTo-Json
...
try {
  ...
  Write-Host "  - Duraci??n: $($routineResponse.routine.duration_weeks) semanas" -ForegroundColor Cyan
...
try {
  ...
  Write-Host "  - Calor??as diarias: $($nutritionResponse.plan.daily_calories)" -ForegroundColor Cyan
...
$chatBody = @{
  message = "??Cu??l es mi plan de entrenamiento principal?"
} | ConvertTo-Json
...
try {
  ...
  Write-Host "??? Tip del d??a:" -ForegroundColor Green
...
Write-Host "??? TEST COMPLETADO CON ??XITO" -ForegroundColor Green
Write-Host "???????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????" -ForegroundColor Cyan
