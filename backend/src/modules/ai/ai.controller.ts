...
export class AIController {
  ...
  private static async buildActionProposalFromChat(
    ...
    const asksNewRoutine =
      /((nueva|otra)\s+rutina|cambiar\s+(mi\s+)?rutina|actualizar\s+(mi\s+)?rutina|reemplazar\s+(mi\s+)?rutina|crea(?:r|me)?\s+(una\s+)?rutina|genera(?:r|me)?\s+(una\s+)?rutina|hazme\s+(una\s+)?rutina|dame\s+(una\s+)?rutina|arma(?:r|me)?\s+(una\s+)?rutina|disena(?:r|me)?\s+(una\s+)?rutina|quiero\s+(una\s+)?rutina|necesito\s+(una\s+)?rutina|recomiend(?:a|ame)\s+(una\s+)?rutina|plan\s+de\s+entrenamiento|entrenamiento\s+semanal|aplica(?:r|la|la)?\s+(esta\s+|la\s+|mi\s+)?rutina|carga(?:r|la)?\s+(esta\s+|la\s+|mi\s+)?rutina|agrega(?:r|la)?\s+(esta\s+|la\s+|mi\s+)?rutina|pon(?:er|la)?\s+(esta\s+|la\s+|mi\s+)?rutina|traslad(?:a|ar|ala)\s+(esta\s+|la\s+|mi\s+)?rutina|guard(?:a|ar|ala)\s+(esta\s+|la\s+|mi\s+)?rutina|mov(?:er|erla|ila)\s+(esta\s+|la\s+|mi\s+)?rutina|pas(?:a|ar|ala)\s+(esta\s+|la\s+|mi\s+)?rutina|lléva(?:la)?\s+(esta\s+|la\s+|mi\s+)?rutina|lleva(?:la)?\s+(esta\s+|la\s+|mi\s+)?rutina|subir\s+(esta\s+|la\s+|mi\s+)?rutina|sube\s+(esta\s+|la\s+|mi\s+)?rutina|rutina.*seccion|seccion.*rutina|mis\s+rutinas|a\s+la\s+seccion\s+de\s+rutina|en\s+mi\s+seccion)/.test(
        normalizedMessage,
      );
